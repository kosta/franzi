...
impl ToKafkaBytes for KafkaString {
    ...
    fn write(&self, bytes: &mut dyn BufMut) {
        // TODO: Overflow check?
        bytes.put_i16(self.0.len() as i16);
    ...
    fn read(bytes: &mut Bytes) -> Result<Self, FromBytesError> {
        ...
        while len > 0 {
            ...
            if slice_len >= len {
                ...
                s.put(slice);
                bytes.advance(slice_len);
                len -= slice_len;
            }
        }
        let s = KafkaString(s.freeze());
        Ok(Some(s))
    }
}
...
impl FromKafkaBytes for VarintBytes {
    fn read(bytes: &mut Bytes) -> Result<Self, FromBytesError> {
        let len: i64 = varint::read_vari64(bytes)?;
        if len <= 0 {
...
mod tests {
    ...
    pub fn varint_from_bytes() {
        ...
        assert_eq!(
            ...
            VarintBytes::read(&mut Bytes::from(INPUT)).unwrap(),
        );
    }
}
